...
def rejestracja():
    ...
    browser.get(dane.doradcaksiegowego_url)
    ...
    time.sleep(2)
    while war == 0:
        if browser.current_url == 'https://www.doradcaksiegowego.pl/rejestracja':
            war += 1
        else:
            help += 1
...
def zamowienie():
    browser.get('https://www.doradcaksiegowego.pl/wybor-planu')
    print("przechodzimy do zamówienia, na doradcaksięgowego wybierzemy pakiet z osobisty na miesiąc")
    browser.find_element_by_class_name("cc-compliance").click()
    time.sleep(5)
    try:
        browser.find_element_by_xpath("//a[contains(@href, '/koszyk/zamow/49')]").click()  # na miesiąc
    ...
    time.sleep(2)
    browser.execute_script("document.getElementById('term_346').checked = true;"
